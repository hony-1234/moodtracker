...
img_path = os.path.join(folder, "xinxin_body_base_v3.png")
...
if os.path.exists(img_path):
    ...
    # Run BFS flood fill from the boundary to identify the "outside" transparent pixels.
    # Any transparent pixel that is NOT connected to the outside boundary is an "internal hole".
    ...
    # Now, any transparent pixel (alpha == 0) that is NOT visited is an internal hole!
    internal_holes = []
    for y in range(height):
        for x in range(width):
            if pixels[x, y][3] == 0 and not visited[y][x]:
