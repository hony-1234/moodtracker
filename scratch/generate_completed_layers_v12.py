...
with Image.open(img_path) as img:
    ...
    print("  - Inpainting perfect continuous face skin...")
    # Best fit face ellipse: Center=(355, 536), Radii: a=199, b=174
    xc, yc, a_rad, b_rad = 355, 536, 199, 174
    
    # Paint skin-tone inside the entire face ellipse for y >= 530
    for y in range(530, 712):
        for x in range(width):
            val = ((x - xc) / a_rad) ** 2 + ((y - yc) / b_rad) ** 2
            if val <= 1.0:
                base_pixels[x, y] = skin_tone
                
    # 6F. Draw reconstructed smooth round face chin outline to complete the head circle perfectly!
    print("  - Drawing reconstructed continuous round face outline...")
    # We draw the outline along the ellipse edge from y = 640 to y = 710
    draw_base = ImageDraw.Draw(body_base)
    for y in range(640, 711):
