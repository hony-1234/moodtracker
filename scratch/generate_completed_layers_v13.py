...
with Image.open(img_path) as img:
    ...
    for y in range(lh):
        for x in range(lw):
            abs_x = box_left_hand[0] + x
            abs_y = box_left_hand[1] + y
            r, g, b, a = left_hand_raw.getpixel((x, y))
            if a == 0:
                continue
            ...
            if (x, y) in hand_skin_l:
                arm_cores_l.add((x, y)) # 1. Hand skin component is 100% ARM
            elif (x, y) in face_skin_l:
                face_cores_l.add((x, y)) # 2. Cheek skin component is 100% FACE
            elif is_red:
                if abs_x < 240:
                    arm_cores_l.add((x, y)) # 3. Sleeve red is ARM
                else:
                    face_cores_l.add((x, y)) # 3. Central body dress red is FACE
            else:
                if abs_y < 650:
                    if abs_x >= 220:
                        face_cores_l.add((x, y)) # 4. Helmet/hair at top-right is FACE
                    else:
                        arm_cores_l.add((x, y))  # 4. Sleeve outline at top-left is ARM
                else:
                    if abs_x < 240:
                        arm_cores_l.add((x, y)) # 5. Outer sleeve outline is ARM
                    else:
                        face_cores_l.add((x, y)) # 5. Face/body outline is FACE
    ...
    for y in range(rh):
        for x in range(rw):
            abs_x = box_right_hand[0] + x
            abs_y = box_right_hand[1] + y
            r, g, b, a = right_hand_raw.getpixel((x, y))
            if a == 0:
                continue
            is_red = (r > 150 and g < 100 and b < 100)
            ...
            if (x, y) in hand_skin_r:
                arm_cores_r.add((x, y)) # 1. Hand skin component is 100% ARM
            elif (x, y) in face_skin_r:
                face_cores_r.add((x, y)) # 2. Cheek skin component is 100% FACE
            elif is_red:
                if abs_x > 460:
                    arm_cores_r.add((x, y)) # 3. Sleeve red is ARM
                else:
                    face_cores_r.add((x, y)) # 3. Central body dress red is FACE
            else:
                if abs_y < 650:
                    if abs_x <= 480:
                        face_cores_r.add((x, y)) # 4. Helmet/hair is FACE
                    else:
                        arm_cores_r.add((x, y))  # 4. Sleeve outline is ARM
                else:
                    if abs_x > 460:
                        arm_cores_r.add((x, y)) # 5. Outer sleeve outline is ARM
                    else:
                        face_cores_r.add((x, y)) # 5. Face/body outline is FACE
    ...
    # 6C. Fill eye regions with skin-tone (so the blinking overlays work with no double eyes)
    ...
    print("  - Erasing entire Left and Right arms from the body base...")
    ...
    # 6E. Inpaint and draw mathematically perfect face cheek and chin
    # CRITICAL: We only fill transparent or semi-transparent pixels inside the fitted face ellipse.
    # This prevents overwriting the original mouth, nose, smile, blush, and center cheek skin!
    print("  - Inpainting perfect continuous face skin ONLY in transparent gaps...")
    # Best fit face ellipse: Center=(355, 536), Radii: a=199, b=174
    ...
    # 6F. Draw reconstructed smooth round face contour/outline along the ellipse edge where hands were
    print("  - Drawing reconstructed continuous round face outline...")
    # Draw outline along ellipse edge from y = 580 to y = 711 on both left and right sides
    ...
    for y in range(580, 712):
        ...
        if val >= 0:
            ...
            # Left side outline pixel (radius=4, thickness=8)
            draw_base.ellipse([xl - 4, y - 4, xl + 4, y + 4], fill=outline_color)
            # Right side outline pixel (radius=4, thickness=8)
            draw_base.ellipse([xr - 4, y - 4, xr + 4, y + 4], fill=outline_color)
            
    # Draw perfect completed shoulder joints backing circles (radius 32) on body base to prevent pivot gaps
    ...
    print("xinxin_body_base_v5.png successfully created with complete cheeks, mouth, nose, and blush!")
    
print("\n🎉 ALL SEAMLESS, DETACHED AND STUCK-FACE mascot LAYERS SUCCESSFULLY RE-GENERATED!")
sys.exit(0)
