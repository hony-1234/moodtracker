...
with Image.open(img_path) as img:
    ...
    # v8 Flame: crop at y=212 to fit exactly. 
    box_fire = [171, 50, 593, 212]          
    # v8 Left Hand: expanded rightwards to x=400 (from 364) to avoid clipping radius=32 circle
    box_left_hand = [81, 600, 400, 800]      
    # v8 Right Hand: expanded leftwards to x=340 (from 366) to avoid clipping radius=32 circle
    box_right_hand = [340, 600, 631, 800]    
    # Keep legs and eye boxes as-is (working perfectly!)
    ...
    # Broad left arm polygon that includes pivot area perfectly and prevents cut-offs
    # pivot_l inside box is at (269, 170). Joint circle of radius 32 spans x:[237, 301], y:[138, 202]
    left_polygon = [
        (0, 0),
        (150, 0),
        (269, 138),  # 32px above pivot
        (301, 170),  # 32px right of pivot (well within box width 319!)
        (269, 202),  # 32px below pivot (well within box height 200!)
        (150, 200),
        (0, 120),
        (0, 0)
    ]
    ...
    # Broad right arm polygon that includes pivot area perfectly and prevents cut-offs
    # pivot_r inside box is at (39, 170). Joint circle of radius 32 spans x:[7, 71], y:[138, 202]
    right_polygon = [
        (rw, 0),
        (140, 0),
        (39, 138),   # 32px above pivot
        (7, 170),    # 32px left of pivot (well within box x >= 0)
        (39, 202),   # 32px below pivot (well within box height 200!)
        (140, 200),
        (rw, 120),
        (rw, 0)
    ]
    ...
    # 6D. Cleanly remove arms, preserve face/neck 100%, and inpaint shoulder backing circles
    print("  - Cleanly removing arms, preserving chin/neck, and inpainting shoulder joints...")
    for y in range(600, 801):
        # Calculate torso flare boundaries only for y >= 700 (dress area)
        ...
        for x in range(width):
            ...
            if a > 0:
                if y < 700:
                    # In chin/neck area (y < 700): preserve everything except the far outer arm sleeves!
                    # Left hand outer sleeve is x < 180, right hand outer sleeve is x > 490
                    if x < 180 or x > 490:
                        base_pixels[x, y] = (0, 0, 0, 0)
                else:
                    # In dress area (y >= 700): erase arms outside torso bounds
                    if x < x_left or x > x_right:
                        base_pixels[x, y] = (0, 0, 0, 0)
                    else:
                        # Inside torso: fill with dress red (dress is red at y >= 710, neck collar is skin tone)
                        if y < 710:
                            base_pixels[x, y] = skin_tone
                        else:
                            base_pixels[x, y] = dress_red
                            
    # Now draw fully solid, overlapping dress_red circular shoulder backing backings (radius 32)
    # centered at left pivot (350, 770) and right pivot (379, 770) on the body base.
    # This guarantees that even when the arms rotate, there is solid dress background underneath them.
    draw_base = ImageDraw.Draw(body_base)
    # Left shoulder backing
    draw_base.ellipse(
        [350 - 32, 770 - 32, 350 + 32, 770 + 32],
        fill=dress_red
    )
    # Right shoulder backing
    draw_base.ellipse(
        [379 - 32, 770 - 32, 379 + 32, 770 + 32],
        fill=dress_red
    )
...
print("\n🎉 ALL SEAMLESS, COMPLETED, AND CLIPPING-FREE LAYERS SUCCESSFULLY GENERATED!")
sys.exit(0)
