...
with Image.open(img_path) as img:
    ...
    # We find red pixels inside the left crop box
    red_pixels = []
    red_set = set()
    for y in range(lh):
        for x in range(lw):
    ...
    print(f"Found {len(components_sorted)} red components inside Left Crop Box.")
    for idx, comp in enumerate(components_sorted[:10]):
        ...
        abs_cx = centroid_x + box_l[0]
        abs_cy = centroid_y + box_l[1]
