...
with Image.open(img_path) as img:
    ...
    # Let's find Component 1 (face cheek) and Component 2 (hand) of skin pixels in left hand box
    w, h = img.size
    
    # Let's define skin-tone pixels in absolute coordinates of the left box
    skin_pixels = []
    skin_set = set()
    ...
    # Let's print the minimum distance and the path between the two components
    min_dist = 999999
    nearest_pair = None
    for fp in face_skin:
        for hp in hand_skin:
    ...
    print(f"Minimum distance between Left Face Cheek skin and Left Hand skin: {actual_dist:.2f} pixels")
    print(f"Nearest pair: Face {nearest_pair[0]} and Hand {nearest_pair[1]}")
    
    # Let's analyze the pixel coordinates and colors on a straight line or box containing these two points
    # Let's see what lies between them.
