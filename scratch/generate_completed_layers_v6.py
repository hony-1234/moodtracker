...
with Image.open(img_path) as img:
    ...
    box_fire = [171, 50, 593, 280]          # Extended down from 250 to 280
    box_left_hand = [81, 600, 364, 800]      # Same box, we will draw rounded joint
    box_right_hand = [366, 600, 631, 800]    # Same box, we will draw rounded joint
    box_legs = [218, 760, 509, 955]          # Extended up from 800 to 760
    ...
    # BFS from (146, 105) because box top moved up by 40px (800 to 760), shifting the white pixel's relative y by +40
    ...
    # Clear the region from row 0 to 70 and project the leg sticks upwards
    print("  - Projecting leg sticks upwards and clearing empty spaces...")
    source_y = 70 # y = 830 in original image
    
    # Store source pixels first so we don't overwrite them during copying
    left_source = [leg_pixels[x, source_y] for x in range(90, 111)]
    right_source = [leg_pixels[x, source_y] for x in range(190, 211)]
    
    for y in range(0, source_y):
        for x in range(legw):
            if 90 <= x <= 110:
                leg_pixels[x, y] = left_source[x - 90]
            elif 190 <= x <= 210:
                leg_pixels[x, y] = right_source[x - 190]
            else:
    ...
    # Erase flame above helmet top completely on the body base (y < 220)
    print("  - Erasing flame from body base (y < 220)...")
    for y in range(0, 220):
        for x in range(width):
            base_pixels[x, y] = (0, 0, 0, 0)
            
    # Erase legs completely on the body base below the skirt (y >= 831)
