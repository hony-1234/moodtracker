...
export interface LandingClassicProps {
...
export function LandingClassic({ setViewState, setPrivacyModalVisible, onSwitchToCartoon }: LandingClassicProps) {
    ...
    >
      {onSwitchToCartoon && (
        <div className="flex justify-center mb-6">
          <button
            type="button"
            onClick={onSwitchToCartoon}
            className="flex items-center gap-2 bg-gradient-to-r from-amber-500 to-indigo-600 hover:from-amber-600 hover:to-indigo-700 text-white font-bold px-5 py-2.5 rounded-full shadow-lg border border-white/60 transition-all hover:scale-105 active:scale-95 text-xs md:text-sm cursor-pointer"
          >
            <Sparkles className="w-4 h-4 text-yellow-300" />
            <span>切換至沉浸校園全景版面 🏫</span>
          </button>
        </div>
      )}
      ...
      <div className="mb-10 max-w-sm mx-auto space-y-4">
        ...
        >
          ...
          >
            ...
            <div 
              title="點擊前往學生積點獎勵系統"
              className="z-10 bg-white/85 backdrop-blur-xs px-4 py-2 rounded-2xl border border-violet-100 shadow-3xs text-xs font-black text-violet-700 transition-colors hover:bg-white flex items-center gap-1.5"
            >
              <span>前往學生積點獎勵系統 ↗</span>
            </div>
          </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
          <div className="w-20 h-20 bg-amber-50 group-hover:bg-amber-100 rounded-full mb-4 overflow-hidden border-2 border-amber-200/50 group-hover:border-amber-400 transition-all flex items-center justify-center p-1">
            <img 
              src={encodeURI("/學校圖檔/吉祥物/enen_reading.png")} 
              alt="GCCPS 吉祥物恩恩" 
              className="w-16 h-16 object-contain"
              onError={(e) => {
                (e.target as HTMLElement).style.display = 'none';
          ...
          <p className="text-xs text-slate-500 mb-4 max-w-xs leading-relaxed">
            老師選擇任教班級並輸入班級密碼即可進入（無需 Gmail 登入），管理班級情緒報告、日記與分析。
          </p>
